...
export default function WalletTopup({ balance, refresh, notify }: { balance: number; refresh: () => Promise<void>; notify: (message: string) => void }) {
  ...
  const { amount, setAmount, commit: commitAmount, bind: amountBind } = useAmountInput(100000, { max: 5_000_000 })
  const [busy, setBusy] = useState(false)
  const topup = async () => {
    const value = commitAmount()
    setBusy(true)
    try {
      const result = await api<{ message: string; balance: number }>('/api/wallet/topup', { method: 'POST', body: JSON.stringify({ amount: value }) })
