...
function App() {
  ...
  const requireLogin = (callback?: () => void) => {
    ...
    if (me.user.sessionMode === 'demo') {
      notify('체험 모드에서는 AI 상담과 사장님 샘플 업로드만 가능해요. 투자·쿠폰 교환은 회원가입 또는 실제 로그인 후 이용해주세요.')
      return false
    }
    callback?.()
    return true
  ...
  return <header className="site-header">
    <div className="header-inner">
      ...
      <div className="header-actions">
        ...
        {user ? <div className="user-menu"><NavLink to={user.sessionMode === 'demo' && user.role === 'owner' ? '/owner' : '/my'} className="avatar">{user.name.slice(0, 1)}</NavLink><div className="user-copy hide-mobile"><b>{user.name}</b><span>{user.sessionMode === 'demo' ? '저장 안 되는 체험 모드' : user.role === 'owner' ? '사장님' : compactWon(user.cash)}</span></div><button className="icon-button hide-mobile" onClick={onLogout} aria-label="로그아웃"><LogOut size={18} /></button></div> : <button className="button small" onClick={onLogin}>로그인</button>}
