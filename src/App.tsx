...
function AuthModal({ onClose, onAuth, notify }: { onClose: () => void; onAuth: (token: string, destination?: string) => Promise<void>; notify: (m: string) => void }) {
  ...
  const demo = async (type: Role) => {
    setBusy(true)
    try {
      // 세 체험 모두 버튼을 누를 때마다 새 임시 세션으로 시작한다.
      // 운영자 체험도 예외가 아니다. 예전에는 진짜 관리자 계정으로 로그인시켜서
      // 체험자가 실제 회원을 정지시키거나 실제 심사를 승인·거절할 수 있었고,
      // 동시에 여러 명이 들어오면 서로의 변경까지 덮어썼다.
      // 이제 운영자 체험은 원장을 읽기만 하고, 누른 변경은 자기 세션에만 남는다.
