...
function AuthModal({ onClose, onAuth, notify }: { onClose: () => void; onAuth: (token: string, destination?: string) => Promise<void>; notify: (m: string) => void }) {
  ...
  const demo = async (type: Role) => {
    setBusy(true)
    try {
      // 투자자·사장님 체험은 버튼을 누를 때마다 새 임시 원장으로 시작한다.
      // 관리자는 공유 원장을 검토해야 하므로 기존 심사용 관리자 계정으로 로그인한다.
      const result = type === 'admin'
        ? await api<{ token: string }>('/api/auth/login', { method: 'POST', body: JSON.stringify({ email: 'admin@meoktu.demo', password: 'demo1234!' }) })
        : await api<{ token: string }>('/api/auth/demo', { method: 'POST', body: JSON.stringify({ role: type }) })
