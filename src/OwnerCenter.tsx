...
import { ArrowLeft, BadgeCheck, Banknote, Building2, Check, Database, FileSpreadsheet, Landmark, Link2, LockKeyhole, ReceiptText, ShieldCheck, Store, UploadCloud, UserCheck, Users, type LucideIcon } from 'lucide-react'
...
const uploadOptions: UploadOption[] = [
  { id: 'business', icon: Building2, title: '사업자등록 자료', exact: '사업자등록증명 또는 사업자등록증 사본 1부', columns: '확인 항목: 상호, 대표자, 개업일, 사업장 주소, 업태·종목', accept: '.pdf,.jpg,.jpeg,.png', required: true },
  ...
  { id: 'pos', icon: FileSpreadsheet, title: 'POS 매출 원자료', exact: '최근 12개월 일별 주문·결제 내역 CSV 또는 XLSX', columns: '필수 열: 영업일, 결제시각, 주문금액, 결제수단, 메뉴·수량, 취소·환불액', accept: '.csv,.xlsx,.xls', required: true },
  { id: 'account', icon: Landmark, title: '사업용 계좌 거래내역', exact: '최근 12개월 입출금 거래내역 CSV·XLSX 또는 은행 발급 PDF', columns: '필수 항목: 거래일시, 입출금액, 잔액, 거래상대방·적요. 계좌번호는 뒤 4자리 외 마스킹 권장', accept: '.csv,.xlsx,.xls,.pdf', required: true },
  { id: 'card', icon: ReceiptText, title: '카드·VAN·PG 정산자료', exact: '최근 12개월 카드 승인내역과 입금 정산내역', columns: '필수 항목: 승인일, 승인금액, 취소금액, 수수료, 정산일, 실제 입금액', accept: '.csv,.xlsx,.xls,.pdf' },
...
export default function OwnerCenter({ me, onLogin, refresh, notify }: { me: MeState | null; onLogin: () => void; refresh: () => Promise<void>; notify: (message: string) => void }) {
  ...
  const selectFile = (sourceId: string, event: ChangeEvent<HTMLInputElement>) => {
  ...
  return <div className="owner-page owner-v2">
    <section className="owner-page-hero">
      <div><span className="eyebrow light"><Store /> 먹투 사장님 센터</span><h1>숫자를 적는 대신,<br /><em>원천데이터를 연결해요.</em></h1><p>POS·계좌·카드·세무·상권을 교차검증해 먹투가 성장성과 현금흐름을 자동 계산합니다.</p><div className="owner-values"><span><Check /> 자기신고 매출 0개</span><span><Check /> 3중 데이터 검증</span><span><Check /> 부족한 자료는 미산정</span></div></div>
      <div className="review-flow data-flow"><b>펀딩 등록 흐름</b>{['사업자·대표자 인증','자료별 원본 업로드','최소 필수 동의 확인','먹투 자동 지표 계산','성장성·위험 심사','펀딩 등록과 공개범위 선택'].map((title,index) => <div key={title}><span>{index+1}</span><p>{title}</p><Check /></div>)}</div>
    </section>
    <div className="owner-body">
      ...
      {result ? <section className="source-review-result">
        ...
        <fieldset disabled={!owner}>
          ...
          <section className="form-section">
            <div className="form-section-title"><span>2</span><div><h3>자료별 개별 업로드</h3><p>동의 버튼을 대신해 실제로 분석할 자료만 하나씩 선택합니다. 필수 4개 외 자료는 가점 또는 교차검증용입니다.</p></div></div>
            <div className="source-progress"><div><b>{uploadedCount}개</b><span>자료 선택</span></div><div className="progress-track"><i style={{ width: `${Math.min(100, uploadedCount / uploadOptions.length * 100)}%` }} /></div><small>필수: 사업자등록·영업신고·POS·사업계좌 / 나머지는 해당되는 업장만 제출</small></div>
            <div className="document-upload-grid">{uploadOptions.map((option) => <DocumentUploadCard key={option.id} option={option} fileName={uploadedFiles[option.id]} onChange={(event) => selectFile(option.id, event)} />)}</div>
            ...
            <p className="mvp-source-note">현재 시연 버전은 기본 신청 시 파일명과 제출 종류만 저장합니다. 이미지에서 ‘AI 문서 판독’을 직접 누른 경우에만 서버를 통해 설정된 AI로 전송하며, 판독 결과만 저장하고 원본 이미지는 저장하지 않습니다. 실제 서비스에서는 암호화 저장·접근통제와 별도 동의·보유정책이 필요합니다.</p>
          </section>
          ...
          <button className="button full huge" disabled={submitting || !identityVerified}>{submitting ? '원천데이터를 교차검증하고 있어요...' : !identityVerified ? '대표자 본인인증을 먼저 완료해주세요' : '먹투 자동분석 시작'} <Database /></button>
...
function DocumentUploadCard({ option, fileName, onChange }: { option: UploadOption; fileName?: string; onChange: (event: ChangeEvent<HTMLInputElement>) => void }) {
  const Icon = option.icon
  return <label className={`document-upload-card ${fileName ? 'uploaded' : ''}`}>
    <span className="document-icon"><Icon /></span>
    <div className="document-copy"><span className={option.required ? 'required' : 'optional'}>{option.required ? '필수 제출' : '선택 제출'}</span><b>{option.title}</b><p>{option.exact}</p><small>{option.columns}</small>{fileName && <strong><Check /> {fileName}</strong>}</div>
    <span className="document-action"><UploadCloud />{fileName ? '다시 선택' : '파일 선택'}</span>
    <input type="file" name={`document-${option.id}`} accept={option.accept} required={option.required} onChange={onChange} />
  </label>
}
