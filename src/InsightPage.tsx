...
export default function InsightPage({ state, onSelect, notify }: { state: PublicState; onSelect: (restaurant: Restaurant) => void; notify: (message: string) => void }) {
  const [messages, setMessages] = useState<Array<{ role: 'user' | 'ai'; text: string }>>([{ role: 'ai', text: '안녕하세요! SG-LLM 생성형 AI 상담원이에요. 식당의 메뉴·리뷰·공개 매출·보너스 이력까지 물어보세요. 😊' }])
  ...
  const ask = async (suggestion?: string) => {
    ...
    try { const result = await api<{ answer: string; mode: string }>('/api/ai/chat', { method: 'POST', body: JSON.stringify({ question: value }) }); setOnline(result.mode === 'generative-ai'); setMessages((current) => [...current, { role: 'ai', text: result.answer }]) }
  ...
  return <div className="page-wrap">
    ...
    </section><aside className="chat-panel"><div className="chat-head"><span className="bot-avatar"><Bot /></span><div><b>먹투 생성형 AI 상담원</b><small><i /> {online ? 'SG-LLM · 온라인' : 'SG-LLM · 연결 확인 필요'}</small></div></div><div className="chat-messages">{messages.map((message, index) => <div className={`chat-bubble ${message.role}`} key={index}>{message.text}</div>)}{asking && <div className="chat-bubble ai typing">···</div>}</div><div className="suggestions">{['최초 투자자 혜택 설명해줘','매출 공개 식당 비교해줘','소복소복 리뷰와 강점 알려줘'].map((item) => <button key={item} onClick={() => ask(item)}>{item}</button>)}</div><form className="chat-input" onSubmit={(event) => { event.preventDefault(); ask() }}><input value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="궁금한 것을 물어보세요" /><button><ArrowRight /></button></form><p className="ai-disclaimer">SG-LLM이 생성한 답변은 참고용이며 투자 권유가 아닙니다.</p></aside></div>
