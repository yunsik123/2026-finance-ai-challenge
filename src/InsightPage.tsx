  ...
  return <div className="page-wrap insight-page">
    ...
    <div className="insight-layout"><section><section className="ai-compare">
        ...
        {selected.length < 2 ? <div className="compare-empty"><Scale /><b>비교할 가게를 2개 이상 선택해주세요.</b><span>최대 3개까지 한눈에 비교할 수 있어요.</span></div> : <>
          ...
          <div className="compare-explanations"><div className="compare-explanation-head"><Sparkles /><div><span>AI 공개정보 해석</span><h3>수치가 보여주는 특징</h3></div>{interpreting ? <em className="insight-status"><Loader2 className="spin" /> 해석 중</em> : interpretation?.provider === 'google-vertex-ai' ? <em className="insight-status ai"><Bot /> AI 분석</em> : interpretation ? <em className="insight-status">자동 규칙 요약</em> : null}</div>
