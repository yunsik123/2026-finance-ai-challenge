    ...
    <div className="my-coupons coupon-wallet-grid">{visible.map((coupon) => {
      ...
      return <article className={`my-coupon ${coupon.status} ${expiring ? 'expiring' : ''}`} key={coupon.id}>
        <div className="coupon-ticket-main">
          <div className="coupon-card-top"><span className="coupon-food">{coupon.restaurant?.emoji || '🎟️'}</span><div><small>{coupon.type === 'dividend' ? '식당 감사 쿠폰' : coupon.type === 'etf' ? '지역·음식 테마' : coupon.fundId ? '투자 혜택' : '가입 축하 쿠폰'}</small><h3>{coupon.restaurant?.name || coupon.title}</h3></div><em className={`coupon-status ${coupon.status}`}>{statusLabel[coupon.status]}</em></div>
