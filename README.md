...
일반 사용자 로그인 모달에는 운영자 선택지가 노출되지 않으며, 투자자와 소상공인 계정으로 즉시 진입할 수 있다.

간편 시작(로그인 이름/아이디) 및 이메일 가입 모두 서버리스 Direct Auth(`email_confirm: true`)를 연동하여, Supabase의 **Email Rate Limit(메일 발송 제한)** 없이 가입 즉시 토큰을 발급받아 시작할 수 있다.
