...
export const sitePages: SitePage[] = [
  ...
  {
    id: 'page:owner', name: '사장님 센터', route: '/owner/store', menuPath: '상단 메뉴 > 사장님 센터',
...
export const uiTasks: UiTask[] = [
  {
    ...
    steps: [
      ...
      '신청서는 네 단계로 나뉘어 있고 단계마다 화면이 실제로 넘어가요(주소도 /owner/store → upload → reading → plan 으로 바뀌고, 브라우저 뒤로 가기로 앞 단계에 돌아갈 수 있어요).',
      '“가게 정보” 단계에서 상호명·대표자명·사업자등록번호·영업신고번호·사업장 주소를 적고 “휴대전화로 대표자 본인인증”을 눌러요.',
      '“다음”을 누르면 “자료 올리기” 단계예요. 맨 위 “가지고 계신 자료를 그냥 올려주세요” 상자에 사진·PDF·엑셀·CSV 를 그대로 올리면 어떤 자료인지 먹투가 알아보고 알맞은 칸에 넣어줘요.',
      '“읽은 값 확인” 단계에서는 서류에서 읽은 상호·사업자번호·금액이 맞는지 “맞아요” 또는 “고치기”로 확인해요.',
      '마지막 “동의와 계획” 단계에서 필수 고지의 “전문 보기”를 눌러 동의하고 희망 펀딩액·자금 사용계획을 작성해요.',
      '맨 아래 “먹투 자동분석 시작” 버튼을 누르면 결과 화면(/owner/result)으로 넘어가면서 예비심사 결과와 자료 일치도가 나와요.',
  ...
  {
    // id 는 페이지별로 다르게 둔다. 'task:ai-consult' 는 AI 인사이트 화면의 상담창이 이미 쓰고 있다.
    id: 'task:ai-consult-widget', intent: 'AI와 상담하기 (떠 있는 버튼)', pageId: 'page:ai-widget',
    keywords: ['ai상담', 'ai에게', '상담하기', '챗봇', '물어보', '질문하', '도움받', '상담창', '문의하기전에'],
    steps: [
      '어느 화면에서든 오른쪽 아래에 떠 있는 “AI와 상담하기” 버튼을 눌러요.',
      '창이 열리면 궁금한 것을 그대로 적어 보내면 됩니다. 로그인하면 내 투자·내 가게 현황까지 함께 답해줘요.',
      '역할별 추천 질문을 누르면 무엇을 물어볼 수 있는지 바로 확인할 수 있어요.',
      'AI가 답할 수 없는 계정·거래 문제는 상단 메뉴 “신고·문의”에서 1:1 문의로 접수해주세요.',
    ],
    note: 'AI 답변은 공개된 정보와 내 원장 기준의 안내이고 투자 권유가 아닙니다.',
  },
  {
    id: 'task:admin-console', intent: '운영센터 들어가기', pageId: 'page:admin',
    keywords: ['운영센터', '관리자', '어드민', '심사승인', '회원관리', '관리자화면'],
    steps: [
      '오른쪽 위 “로그인” 버튼에서 운영자 계정으로 로그인해요.',
      '로그인하면 상단에 “운영센터”가 나타나고, 회원·식당·펀드·심사·리뷰·문의·쿠폰을 한 화면에서 관리할 수 있어요.',
      '심사 목록에서 항목을 누르면 제출 자료와 값별 근거, 자료 일치도를 확인한 뒤 최종 상태를 정할 수 있어요.',
    ],
    note: '운영자 전용 화면입니다. 사장님·투자자 계정에는 보이지 않아요.',
  },
  {
  ...
  },
  {
    ...
    steps: [
      '상단 메뉴 “사장님 센터”로 들어가요.',
      '“자료 올리기” 단계의 “B. 자료가 들어간 칸” 영역 맨 위 “샘플 자료 한 번에 받기”에서 전체 묶음을 내려받을 수 있어요.',
      '각 자료 카드의 “샘플 다운로드” 버튼으로 필요한 파일만 따로 받을 수도 있습니다.',
      '내려받은 파일을 맨 위 “가지고 계신 자료를 그냥 올려주세요” 상자에 올리면 업로드와 자동 분류, 서류 판독을 그대로 체험할 수 있어요.',
    ],
  },
  {
    ...
    steps: [
      ...
      '“자료 올리기” 단계 맨 위 “가지고 계신 자료를 그냥 올려주세요” 상자에 파일을 끌어다 놓거나 “파일 고르기”로 여러 개를 한 번에 올려요. 사진·PDF·엑셀·CSV 를 모두 받습니다.',
      '자료는 요건별로 묶여 있어요. 사업자등록·영업신고·사업용 계좌는 필수이고, 매출 자료는 POS·카드 매출·납세 자료·배달 정산 중 하나만 있으면 됩니다. 각 카드의 “어디서 받나요?”에 발급 창구가 적혀 있어요.',
      '어떤 서류인지 고르지 않아도 됩니다. 올리면 먹투가 무슨 자료인지 판단해 알맞은 칸에 넣고, 무엇을 보고 그렇게 판단했는지 함께 보여줘요. 틀렸으면 그 줄의 목록에서 직접 바꿀 수 있어요.',
      '기관에서 바로 받아오려면 같은 단계의 “A. 제휴기관·마이데이터형 연결”에서 “동의하고 연결”을 누릅니다.',
      '샘플로 전체 흐름을 보려면 “정리된 자료로 보기” 또는 “실제 사장님 자료처럼 보기”를 누르세요. 두 번째 것은 자료가 서로 어긋난 경우에 불일치가 어떻게 잡히는지 보여줍니다.',
      '다음 단계 “읽은 값 확인”에서 서류에서 읽은 값이 맞는지 확인하면 접수 준비가 끝나요.',
    ],
  },
  {
  ...
  {
    id: 'task:document-issuance', intent: '제출 서류 어디서 발급받는지 확인하기', pageId: 'page:owner',
    keywords: ['발급', '어디서받', '어떻게받', '사업자등록증', '영업신고증', '홈택스', '부가세', '거래내역', '준비물', '뭐가필요', '필수서류', '무슨서류'],
    steps: [
      '상단 메뉴 “사장님 센터”로 들어가 “자료 올리기” 단계로 갑니다.',
      '각 자료 카드 위쪽의 “어디서 받나요?”를 누르면 발급 창구와 클릭 순서, 홈페이지 주소가 펼쳐집니다.',
      '필수 자료는 사업자등록 자료·영업신고 자료·사업용 계좌 내역이고, 매출 자료는 POS·카드 매출·납세 자료·배달 정산 중 하나만 있으면 됩니다.',
      '대출이 있으면 같은 단계의 “부채 확인”에서 잔액·금리·월 상환액을 직접 적어주세요. 대출이 없으면 “대출 없어요”를 한 번 누르면 끝납니다.',
    ],
    note: 'POS 자료가 없어도 신청할 수 있습니다. 카드 매출자료나 홈택스 자료로 매출을 확인합니다.',
  },
  {
    id: 'task:fund-use-plan', intent: '자금 사용계획 적기', pageId: 'page:owner',
    keywords: ['자금사용계획', '사용처', '어디에쓸', '자금계획', '사업계획', '얼마씩', '배분'],
    steps: [
      '사장님 센터 신청서의 마지막 “동의와 계획” 단계로 갑니다.',
      '“투자금을 어디에 쓸 계획인가요?”에서 항목(주방설비·인테리어·운전자금 등)과 금액을 한 줄씩 추가합니다.',
      '항목 금액의 합계가 희망 펀딩액과 같아야 접수돼요. “남은 금액을 마지막 항목에 넣기” 버튼으로 맞출 수 있습니다.',
      '아래 설명란에는 그 돈으로 무엇이 달라지는지 두세 문장만 적으면 됩니다. 긴 사업계획서는 필요하지 않아요.',
    ],
    note: '자금 사용계획은 투자자에게 그대로 공개됩니다. 예비평가 점수에는 반영하지 않고 운영자와 투자자의 판단자료로 씁니다.',
  },
  {
    id: 'task:evidence-public', intent: '식당이 낸 자료가 서로 맞는지 확인하기 (투자자)', pageId: 'page:discover',
    keywords: ['자료일치도', '일치도', '검증됐', '믿을수있', '근거', '진짜맞', '교차검증', '자료맞', '어떻게검증'],
    steps: [
      '상단 메뉴의 “식당 발견”에서 보고 싶은 식당 카드를 눌러요.',
      '상세창의 “투자 전 한눈에 보기” 안에 “제출자료 일치도” 카드가 있어요. 점수와 함께 서로 맞춰본 자료가 몇 쌍인지 표시돼요.',
      '맞지 않는 항목이 남아 있으면 그 건수도 함께 보여주고, 운영자가 확인 중임을 알려줘요.',
    ],
    note: '사장님의 개별 금액이나 파일 이름은 공개하지 않고, 몇 쌍을 맞춰봤는지와 점수만 공개합니다. 일치도는 승인 결과나 수익 보장이 아닙니다.',
  },
  {
    id: 'task:evidence-check', intent: '제출한 자료가 서로 맞는지 확인하기', pageId: 'page:owner',
    keywords: ['자료일치도', '일치도', '교차검증', '대조', '근거', '어디서나온', '왜이숫자', '숫자근거', '불일치', '안맞'],
    steps: [
      '소상공인 계정으로 로그인한 뒤 상단 메뉴의 “사장님 센터”로 가요.',
      '자료를 올리고 마지막 단계에서 “먹투 자동분석 시작”을 누르면 결과 화면이 나와요.',
      '결과 화면의 “이 숫자가 어디서 나왔는지 보여드려요” 영역에서 자료 일치도 점수와 서로 맞춰본 자료 쌍을 볼 수 있어요.',
      '지표 줄을 누르면 그 숫자가 어느 파일 몇 행에서 나왔는지, 다른 자료로는 얼마였는지 펼쳐집니다.',
      '이미 접수한 신청은 상단 “마이페이지”의 “제출 자료 검증 결과”에서 같은 내용을 다시 볼 수 있어요.',
    ],
    note: '자료 일치도는 제출 자료끼리 맞춰본 결과이며 승인 결과가 아닙니다. 최종 승인은 운영자가 원본을 확인한 뒤 이뤄져요.',
  },
  {
...
export function siteGraphNodes() {
  const pages = sitePages.map((page) => ({
    ...
    properties: {
      ...
      loginRequired: page.login, purpose: page.purpose.slice(0, 200), actions: page.actions.join(' / ').slice(0, 200),
    },
  }))
  const tasks = uiTasks.map((task) => ({
    ...
    properties: {
      ...
      // 그래프 속성은 짧게 유지한다. Neo4j 로 그대로 나가고, 검색 haystack 도 이 문자열을 훑는다.
      clickPath: task.steps.join(' → ').slice(0, 200),
