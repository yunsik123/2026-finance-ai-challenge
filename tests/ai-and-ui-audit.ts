...
const navigationCases = [
  ...
  ['투자자에게 배당 쿠폰은 어떻게 보내?', ['사장님 센터', '10% 배당 쿠폰 보내기']],
] as const
...
assert(ownerStatus.mode === 'owner-ledger-local' && /\d단계/.test(ownerStatus.answer), '사장님 상담은 본인 심사 원장의 현재 단계를 답해야 합니다.')
...
const emptyBookFund = publicState.funds.find((fund: any) => fund.status === 'trading' && fund.openSellAmount === 0 && fund.openBuyAmount === 0)
assert(emptyBookFund, '빈 대기열에 예약 주문을 만들 거래 중 펀드가 필요합니다.')
